...
export const CONMUTE_TIMER = 'CONMUTE_TIMER';
...
// Swiches the interval Off. Stores the interval state
export function conmuteTimer(timerId) {
  return {
    type: CONMUTE_TIMER,
    payload: {
      timerId
...
// Decreases by 1 the time left
export function decreaseTimeLeft(timerId, timeLeft, questions) {
  if (timeLeft === 0) {
    return (dispatch) => {
      dispatch(conmuteTimer(timerId))
